...
async def handle_help(db: BotDatabase, sender: str, text: str):
    await wa.send_text(
        ...
        "🎬 *Avatar & Content Intelligence*\n"
        f"  avatar setup — Set up face & voice profile\n"
        f"  redo voice — Re-record your voice sample\n"
        f"  avatar video — Create a talking-head video ({ACTION_COSTS['ai_video']} credits)\n"
        f"  pillars — Set your content niche pillars\n"
        f"  content idea — Mine URL/text → grade → Reel/Carousel/Post\n\n"
        "💳 *Credits & Plans*\n"
...
async def _send_onboarding_complete(db: BotDatabase, sender: str):
    ...
    await wa.send_text(
        ...
        "3️⃣  Send *pillars* — set your content niche pillars\n"
        "4️⃣  Send *content idea* — mine a news article → get a Reel/Post\n"
        "5️⃣  Send *subscribe* — view plans for more credits\n\n"
        "*Credit costs:*\n"
        f"  Photo/Video post: {ACTION_COSTS['own_media_post']} | Text post: {ACTION_COSTS['text_post']} | Comment reply: {ACTION_COSTS['comment_reply']} | Avatar video: {ACTION_COSTS['ai_video']}\n\n"
