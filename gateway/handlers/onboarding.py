...
async def handle_help(db: BotDatabase, sender: str, text: str):
    await wa.send_text(
        ...
        "🎬 *Video*\n"
        f"  *video* — Avatar Reels & content pipeline ({ACTION_COSTS['ai_video']} credits)\n"
        "    ↳ Set up face & voice, define content pillars,\n"
        "       mine a news URL → grade → Reel / Carousel / Text Post\n\n"
        "💳 *Credits & Plans*\n"
        ...
        "  *reset* — Fix any stuck session\n\n"
        "⚠️ If unresponsive, send *reset* and retry.",
    )
...
async def _send_onboarding_complete(db: BotDatabase, sender: str):
    ...
    await wa.send_text(
        ...
        "3️⃣  Send *video* — set up your avatar & start creating Reels from news/ideas\n"
        "4️⃣  Send *subscribe* — view plans for more credits\n\n"
        "*Credit costs:*\n"
