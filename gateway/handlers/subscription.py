...
async def handle_credits(db: BotDatabase, sender: str, text: str):
    ...
    await wa.send_text(
        ...
        f"  AI Image: {ACTION_COSTS['ai_image']} credits\n"
        f"  AI Video: {ACTION_COSTS['ai_video']} credits\n"
        f"  Comment reply: {ACTION_COSTS['comment_reply']} credits\n\n"
        + ("Credits reset on your next billing cycle." if is_sub else
           "Want more credits?\n"
           f"  *subscribe* — Monthly plans from S${PLANS['pro']['price_usd']}\n"
           "  *buy* — One-time credit packs from S$4.99\n"
...
async def handle_subscribe(db: BotDatabase, sender: str, text: str):
    ...
    for key in ("pro", "business"):
        ...
        if price_id:
            rows.append({
                "id": f"plan_{key}",
                "title": f"{plan['name']} — S${plan['price_usd']}/mo",
    ...
    if rows:
        await wa.send_interactive_list(
            ...
            f"*Pro* — S${PLANS['pro']['price_usd']}/mo → {PLANS['pro']['credits']:,} credits\n"
            f"*Business* — S${PLANS['business']['price_usd']}/mo → {PLANS['business']['credits']:,} credits\n\n"
...
async def handle_buy_credits(db: BotDatabase, sender: str, text: str):
    ...
    await wa.send_interactive_list(
        ...
        f"  100 credits — S$4.99\n"
        f"  500 credits — S$24.99\n"
        f"  1,500 credits — S$74.99\n"
        f"  5,000 credits — S$200.00\n\n"
        "Local currency will be shown at checkout.",
        "Choose Pack",
